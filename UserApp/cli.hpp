...
//
// 命令集（小写，\r 或 \n 结尾）：
//   xxNN   学习录制到槽 NN（00~07），空闲 100ms 自动保存，15s 超时
//   fsNN   回放槽 NN
//   slots  列出 8 槽占用状态
//   help   列出用法
//
...
{
public:
    Cli(IrStorage& st, IrSignal& sig, IrReceiver& rx, IrTransmitter& tx);
...
private:
    ...
    void printRaw(const IrSignal& sig, uint32_t len); // 带符号逗号分隔打印，末尾 len=段数
